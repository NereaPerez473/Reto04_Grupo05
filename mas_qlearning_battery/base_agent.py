...
class ProducerAgent:
    ...
    def _build_proposal(self, import_price: float) -> EnergyProposal:
        """
        Genera una propuesta usando Q-Learning para seleccionar la estrategia de negociación.
        Adapta el estado dinámicamente según las dimensiones de la Q-Table cargada.
        """
        ...
        # actual tiene el método 'discretize_production' para evitar reinstanciar.
        if self.learner.q_table.ndim == 5 and not hasattr(self.learner, "discretize_production"):
            from strategy_qlearning_negotiation_battery import StrategyQLearning as StrategyNegotiation
            new_learner = StrategyNegotiation()
            new_learner.q_table = self.learner.q_table # Conservar la matriz aprendida
            new_learner.epsilon = self.learner.epsilon # Conservar la política de explotación
        ...
        if self.learner.q_table.ndim == 5:
            ...
            state = self.learner.get_state(
                ...
                battery_soc=self.last_soc
            )
        else:
            # Modelo Competitivo / Cooperativo (3 variables de estado)
            state = self.learner.get_state(
                demand_kw=self.last_demand,
                price_eur_kwh=self.last_price,
                battery_soc=self.last_soc
        ...
        # Elegir acción y convertir a estrategia
        action = self.learner.choose_action(state)
        strategy_name = self.learner.action_to_strategy(action)

        print(f"[{self.name}] t={self.current_step} estrategia={strategy_name}")

        self.last_state = state
        self.last_action = action

        # Preparar los argumentos específicos de las estrategias manipuladoras
        kwargs = {}
