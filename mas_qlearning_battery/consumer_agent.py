...
class ConsumerAgent:
    ...
    def __init__(self, load_csv_path: str, price_csv_path: str,
        ...
        self.load_series = (df_load[LOAD_COL].astype(float)*2.5).values
    ...
    def run(self) -> pd.DataFrame:
        ...
        try:
            for t in range(self.n_steps):
                ...
                # ── NUEVO: Ejecutar física de batería antes del CFP ──── #
                # La batería intenta cubrir la demanda bruta actual.
                # Lo que no pueda cubrir, se convierte en la demanda efectiva.
                delivered_by_battery = self.battery.discharge(demand)
                effective_demand = demand - delivered_by_battery

                # ── Fase 1: CFP ──────────────────────────────────────── #
                cfp_content = {
                    "demand_kw":            round(effective_demand, 4), # ACTUALIZADO
                ...
                if msg_solar is None:
                    print(f"Solar desconectado en timestep {t}")
                    break

                if msg_wind is None:
                    print(f"Wind desconectado en timestep {t}")
                    break
                ...
                allocation = self._decide_allocation(effective_demand, proposals, import_price)
                ...
                # ── Fase 4: Balance físico final y gestión de excedentes ─ #
                renewable_total = solar_delivered + wind_delivered
                
                grid_purchased = 0.0
                battery_charged_kw = 0.0
                curtailed_kw = 0.0  # Energía disipada / perdida

                if renewable_total < effective_demand:
                    # Falta energía renovable -> compramos el déficit a la red
                    grid_purchased = effective_demand - renewable_total
                
                elif renewable_total > effective_demand:
                    # Sobra energía renovable -> intentamos cargar la batería
                    surplus = renewable_total - effective_demand
                    
                    # La batería absorbe hasta su límite de potencia (50kW) y capacidad
                    battery_charged_kw = self.battery.charge(surplus)
                    
                    # Lo que la batería no puede absorber físicamente, se disipa
                    curtailed_kw = surplus - battery_charged_kw

                # Balance económico (la energía disipada reporta 0 € de ingresos)
                ...
                if solar_shortfall > 0 or wind_shortfall > 0:
                    print(
                        f"SHORTFALL t={t} | "
                        f"solar={solar_shortfall:.2f} | "
                        f"wind={wind_shortfall:.2f}"
                    )
        ...
        print(f"  Energía disipada total:    {df['curtailed_energy_kw'].sum():.1f} kWh") # NUEVO
        ...
        df.to_csv(
            results_dir / "mas_qlearning_results.csv",
            index=False
        )
